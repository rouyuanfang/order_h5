<template>
  <div class="normal_page overflow-y f5f5f5" :class="{ app_page: isAndroid }">
    <van-nav-bar title="卡券通知" left-arrow @click-left="onClickLeft">
    </van-nav-bar>
  ...
  methods: {
    ...
    toDetail(item) {
      if (item.subType === 6) {
        ...
        const role = item.userType === 1 ? "branch" : "normal";
        this.$router.push({ name: "coupon-detail", query: { id, role } });
      }
    },
  },
};
